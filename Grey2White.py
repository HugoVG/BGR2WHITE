...
################################################################
#                   TODO                                       #
#             Zorg er voor dat de self kiezer werk             #
################################################################
...
def dysWindow():
    def refreshimg():
        ...
        try:
            ...
            img = load.resize((300,600), Image.ANTIALIAS)
            render = ImageTk.PhotoImage(img)
            img = tk.Label(root,bg="#FFFFFF", image=render)
            img.image = render
            img.place(relx=0.05, rely=0.25, relwidth=0.9, relheight=0.7)
        except IndexError:
            img = tk.Label(root, text="geen foto's meer" )
            img.place(relx=0.05, rely=0.25, relwidth=0.9, relheight=0.7)
        if len(filearray) > 0:
            print("there is another")
            but1.configure(bg="#FFFFFF")
            but1.configure(bg="#FFFFFF")
        else:
            but1.configure(bg="#000000")
            but2.configure(bg="#000000")
    ...
    nlabel = tk.Label(canvas,bg="#80c1ff", text="als knop ingedrukt blijft, wacht, na elke druk op een knop refresh")
    nlabel.place(x=0.5, y=1, relwidth=1)

    but1 = tk.Button(frame,bg="#FFFFFF", text="grijs naar wit",font=1,command=lambda: bgrwhite(filearray[0]))
    but1.place(relx=0.7,relheight=1,relwidth=0.3)
    but2 = tk.Button(frame,bg="#FFFFFF", text="maskeer rondom",font=1,command=lambda: bgr2white(filearray[0]))
    but2.place(relx=0.4,relheight=1,relwidth=0.3)
    but3 = tk.Button(frame,bg="#FFFFFF", text="Refresh",font=1,command=lambda: refreshimg())
